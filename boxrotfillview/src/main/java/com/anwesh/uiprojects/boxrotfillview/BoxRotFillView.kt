...
class BoxRotFillView(ctx : Context) : View(ctx) {
    ...
    data class BRFNode(var i : Int, val state : State = State()) {

        private var next : BRFNode? = null
        private var prev : BRFNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBRFNode(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BRFNode {
            var curr : BRFNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BoxRotFill(var i : Int) {

        private val root : BRFNode = BRFNode(0)
        private var curr : BRFNode = root
